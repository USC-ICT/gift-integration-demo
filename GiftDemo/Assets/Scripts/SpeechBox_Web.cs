...
public class SpeechBox_Web : MonoBehaviour
{
    ...
    {
        get { return m_WebServerUrl; }
        set { m_WebServerUrl = value; }
    }
    ...
    public void SendSpeechMessage(string message)
    {
        ...
        m_Main.SendServerMessage(message);
    ...
    //    TypingEnabled = false;
    //    string url = string.Format("{0}?SpeechUserId={1}&UserMessage={2}&ClientNeedsResponse=true&ParticipantId={3}&IsMicInput=false", WebServerUrl, speechUserId, message.Replace(" ", "%20"), m_Main.ParticipantId);
    //    WWW www = new WWW(url);
    //    Debug.Log(url);
    //    yield return www;
