...
public class CutsceneEditor : TimelineWindow
{
    ...
    const string GenericEventsNoSmartbodyGOPath = "Prefabs/GenericEvents_NoSmartbody.prefab";
    ...
    public class SequencerData
    {
        ...
        public DataType m_DataType = DataType.Xml;
    ...
    {
        get { return m_SequencerData.m_Title; }
    }
    #endregion
    ...
    protected override void Update()
    {
        if (m_CleanRequestState != CleanRequest.None)
        {
            ...
            switch (prevRequest)
            {
                case CleanRequest.OpenBml:
                    FileOpenBML(m_SequencerData.m_Title);
                    break;

                case CleanRequest.OpenXml:
                    FileOpenXML(m_SequencerData.m_Title);
                    break;

                case CleanRequest.OpenBoth:
                    FileOpenBMLXMLPair(m_SequencerData.m_Title);
                    break;

                case CleanRequest.OpenAudioSpeechFile:
                    FileOpenAudioSpeechFile(m_SequencerData.m_Title);
                    break;
            }
            m_CleanRequestState = CleanRequest.None;
        }
        ...
        if (m_SequencerData.m_QueuedFilesToLoad.Count > 0)
        {
            ...
            string fileName = m_SequencerData.m_QueuedFilesToLoad[indexToUse].Replace("\\", "/");
            if (m_SequencerData.m_DataType == SequencerData.DataType.Xml)
            {
                RequestFileOpenBMLXMLPair(fileName);               
            }
            else
            {
                RequestFileOpenAudioSpeechFile(fileName);
            }
    ...
    public void AddCutscene()
    {
        ...
        string goPath = VHUtils.IsSmartbodyAvailable() ? GenericEventsGOPath : GenericEventsNoSmartbodyGOPath;
        GameObject genericEventsObj = (GameObject)PrefabUtility.InstantiatePrefab((GameObject)AssetDatabase.LoadAssetAtPath(MachinimaFolderLocation + goPath, typeof(GameObject)));
        genericEventsObj.transform.parent = cutscene.transform;
        genericEventsObj.name = "GenericEvents";
    ...
    void ShowFileContextMenu()
    {
        ...
        contextMenu.AddSeparator("");
        contextMenu.AddItem(new GUIContent("Audio Speech Prefabs/Importer"), false, OpenImporterWindow, this);
        contextMenu.AddItem(new GUIContent("Audio Speech Prefabs/Single Open"), false, RequestFileOpenAudioSpeechFile, this);
        contextMenu.AddItem(new GUIContent("Audio Speech Prefabs/Batch Open"), false, AudioSpeechFileBatchImport, this);


        contextMenu.AddSeparator("Sequencer/");

        contextMenu.AddItem(new GUIContent("Sequencer/Send Speech"), false, OpenUtteranceWindow, this);

        if (Application.isPlaying)
        {
            contextMenu.AddItem(new GUIContent("Sequencer/Listen to vrSpeak"), m_ListenToNVBG, ListenToNVBG, this);
        }
        else
        {
            contextMenu.AddDisabledItem(new GUIContent("Sequencer/Listen to vrSpeak"));
        }
    ...
    {
        m_SequencerData.m_Title = EditorUtility.OpenFilePanel("Open XML File", string.Format("{0}", PlayerPrefs.GetString(LastFolderPathKey, Application.dataPath)), "xml");
        if (string.IsNullOrEmpty(m_SequencerData.m_Title))
        {
            return;
        }

        PlayerPrefs.SetString(LastFolderPathKey, Path.GetDirectoryName(m_SequencerData.m_Title));
        m_CleanRequestState = CleanRequest.OpenBoth;
    }

    public void RequestFileOpenBMLXMLPair(string filename)
    {
        m_SequencerData.m_Title = filename;
        m_CleanRequestState = CleanRequest.OpenBoth;
    }

    void RequestFileOpenBML(object data)
    {
        m_SequencerData.m_Title = EditorUtility.OpenFilePanel("Open BML File", string.Format("{0}", PlayerPrefs.GetString(LastFolderPathKey, Application.dataPath)), "*.txt;*.bml");
        if (string.IsNullOrEmpty(m_SequencerData.m_Title))
        {
            return;
        }
    ...
    void OpenImporterWindow(object data)
    {
        ImportWindow.Init(position, this);
    }

    public void RequestFileOpenAudioSpeechFile()
    {
        RequestFileOpenAudioSpeechFile(this);
    }

    public void RequestFileOpenAudioSpeechFile(object data)
    {
        m_SequencerData.m_Title = EditorUtility.OpenFilePanel("Open Audio Speech Prefab", string.Format("{0}", PlayerPrefs.GetString(LastFolderPathKey, Application.dataPath)), "prefab");
        if (string.IsNullOrEmpty(m_SequencerData.m_Title))
        {
            return;
        }
        ...
        PlayerPrefs.SetString(LastFolderPathKey, Path.GetDirectoryName(m_SequencerData.m_Title));
        m_CleanRequestState = CleanRequest.OpenAudioSpeechFile;
    }

    public void RequestFileOpenAudioSpeechFile(string filename)
    {
        m_SequencerData.m_Title = filename;
        m_CleanRequestState = CleanRequest.OpenAudioSpeechFile;
    }
    ...
    public AudioSpeechFile FileLoadAudioSpeechFile(string filePathAndName)
    {
        // strip off everything before the assets directory
        int index = filePathAndName.LastIndexOf("/Assets");
        string truncatedPath = filePathAndName.Remove(0, index + 1);
        AudioSpeechFile asf = AssetDatabase.LoadAssetAtPath<AudioSpeechFile>(truncatedPath);
        return asf;
    }

    public void FileOpenAudioSpeechFile(string filePathAndName)
    {
        FileLoadAudioSpeechPrefab(FileLoadAudioSpeechFile(filePathAndName));
    }

    public void FileLoadAudioSpeechPrefab(AudioSpeechFile asf)
    {
        if (asf == null)
        {
            Debug.LogError("Failed: FileLoadAudioSpeechPrefab");
            return;
        }

        if (asf.m_Xml != null && asf.m_LipSyncInfo != null)
        {
            string path = string.Format("{0}/{1}", Application.dataPath.Replace("/Assets", ""), AssetDatabase.GetAssetPath(asf.m_Xml));
            FileOpenBMLXMLPair(path);
        }
        ...
        {
            // have to put this here so that we don't overwrite the current cutscene
            AddCutscene();
            string path = string.Format("{0}/{1}", Application.dataPath.Replace("/Assets", ""), AssetDatabase.GetAssetPath(asf.m_LipSyncInfo));
            FileOpenBML(path);
            
            /*CutsceneEvent audioEvent = new CutsceneEvent(new Rect(), Guid.NewGuid().ToString());
            BMLParser.ChangedCutsceneEventType(GenericEventNames.Mecanim, audioEvent);
            audioEvent.ChangedEventFunction("PlayAudio", 2);
            audioEvent = SequencerIO.AddEvent(audioEvent);
            
           
            audioEvent.Name = audioEvent.m_Params[1].stringData = asf.m_AudioClip.name;
            ChangeCutsceneName(GetSelectedCutscene().CutsceneName, GetSelectedCutscene().CutsceneName.Replace(".bml", ""));*/
        }
    }


    void OpenUtteranceWindow(object data)
    {
        UtteranceWindow.Init(position, this);
    }

    public void ListenToNVBG(bool listen)
    {
        m_ListenToNVBG = listen;
    ...
    void LoadFile(string filePathAndName, bool setTimes)
    {
        ...
        {
            m_SequencerData.m_Title = filePathAndName;
        }
        ...
        string newName = Path.GetFileNameWithoutExtension(Path.GetFileNameWithoutExtension(filePathAndName)); // incase of double extensions
    ...
    void FileBatchImport(object data)
    {
        ...
        if (!string.IsNullOrEmpty(folderName))
        {
            ...
            m_SequencerData.m_DataType = SequencerData.DataType.Xml;
        }
    }

    public void AudioSpeechFileBatchImport()
    {
        AudioSpeechFileBatchImport(null);
    }

    public void AudioSpeechFileBatchImport(object data)
    {
        ...
        if (!string.IsNullOrEmpty(folderName))
        {
            m_SequencerData.m_QueuedFilesToLoad.AddRange(Directory.GetFiles(folderName, "*.prefab"));
            m_SequencerData.m_DataType = SequencerData.DataType.AudioSpeechFile;
        }
    }
