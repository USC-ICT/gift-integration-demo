...
#pragma warning restore 219
        ...
        Canvas canvas = VHUI.CreateCanvas("DebugInfoCanvas", this.gameObject, 100);

        canvas.GetComponent<CanvasGroup>().interactable = false;
        canvas.GetComponent<CanvasGroup>().blocksRaycasts = false;

        m_BackgroundImage = VHUI.CreateImage("Background", canvas.transform);
        ...
        VHUI.StretchToParent(m_BackgroundImage.rectTransform);
        ...
        {
            VHUI.CreateLayoutText("AdditionalLine" + i.ToString(), GeneralTab.transform, m_additionalLinesPersistent[i], Color.white, 1, generalHeight);
        }
        ...
        #else
        CreateText(TextType.Shadows, SystemTab.transform, string.Format("Shadows:{0} FX:{1} MT:{2}", SystemInfo.supportsShadows ? "y" : "n", SystemInfo.supportsImageEffects ? "y" : "n", SystemInfo.graphicsMultiThreaded ? "y" : "n"), systemHeight);
        #endif
        ...
        CreateText(TextType.MonoHeap, SystemTab.transform, string.Format("MonoHeap: {0:N0}", Profiler.GetMonoUsedSizeLong()), systemHeight);
        CreateText(TextType.TempAllocator, SystemTab.transform, string.Format("TempAllocator: {0:N0}", Profiler.GetTempAllocatorSize()), systemHeight);
        CreateText(TextType.AllocatedMemory, SystemTab.transform, string.Format("AllocatedMemory: {0:N0}", Profiler.GetTotalAllocatedMemoryLong()), systemHeight);
        CreateText(TextType.ReservedMemory, SystemTab.transform, string.Format("ReservedMemory: {0:N0}", Profiler.GetTotalReservedMemoryLong()), systemHeight);
        CreateText(TextType.UnusedReservedMemory, SystemTab.transform, string.Format("UnusedReservedMemory: {0:N0}", Profiler.GetTotalUnusedReservedMemoryLong()), systemHeight);
    ...
    {
        m_Tabs[(int)tab] = VHUI.CreateLayoutGroup<VerticalLayoutGroup>(tab.ToString(), parent);
        ...
        VHUI.StretchToParent(layoutRect);
    ...
    Text CreateText(TextType id, Transform parent, string text, float screenPctTall)
    {
        ...
        {
            label = VHUI.CreateLayoutText(id.ToString(), parent, text, Color.white, 1, screenPctTall);
        ...
        VHUI.SetPreferredLayout(layout, 1, screenPctTall);
        label.fontSize = (int)(layout.preferredHeight * 0.7f);

        text = VHUI.InsertBreaks(text, label.font, label.fontSize, (int)((float)Screen.width * 0.9f));
    ...
    void BreakUpLabel(Text label, string[] newLineSeperatedLines, float screenPctTall)
    {
        ...
        {
            Text splitLabel = VHUI.CreateLayoutText(label.name, label.transform.parent, newLineSeperatedLines[i], Color.white, 1, screenPctTall);
            VHUI.SetPreferredLayout(splitLabel.GetComponent<LayoutElement>(), 1, screenPctTall);
    ...
    {
        CreateText(TextType.MonoHeap, m_Labels[TextType.MonoHeap].transform.parent, string.Format("MonoHeap: {0:N0}", Profiler.GetMonoUsedSizeLong()), m_TextScreenHeightPct);
        CreateText(TextType.TempAllocator, m_Labels[TextType.TempAllocator].transform.parent, string.Format("TempAllocator: {0:N0}", Profiler.GetTempAllocatorSize()), m_TextScreenHeightPct);
        CreateText(TextType.AllocatedMemory, m_Labels[TextType.AllocatedMemory].transform.parent, string.Format("AllocatedMemory: {0:N0}", Profiler.GetTotalAllocatedMemoryLong()), m_TextScreenHeightPct);
        CreateText(TextType.ReservedMemory, m_Labels[TextType.ReservedMemory].transform.parent, string.Format("ReservedMemory: {0:N0}", Profiler.GetTotalReservedMemoryLong()), m_TextScreenHeightPct);
        CreateText(TextType.UnusedReservedMemory, m_Labels[TextType.UnusedReservedMemory].transform.parent, string.Format("UnusedReservedMemory: {0:N0}", Profiler.GetTotalUnusedReservedMemoryLong()), m_TextScreenHeightPct);
