...
public class VHMain : MonoBehaviour
{
    ...
    public virtual void OnGUI()
    {
        if (DisplaySubtitles && !string.IsNullOrEmpty(m_subtitleText))
        {
            ...
            VHIMGUI.Label(subtitleTextBackdropPos, m_subtitleText, labelStyle);
            GUI.color = Color.white;
            VHIMGUI.Label(subtitleTextPos, m_subtitleText, labelStyle, Color.yellow);
        ...
        if (DisplayUserDialog && !string.IsNullOrEmpty(m_userDialogText))
        {
            ...
            GUI.color = Color.white;
            VHIMGUI.Label(userDialogTextPos, m_userDialogText, Color.white);
        }
    }
        ...
        if (splitargs.Length > 0)
        {
            if (splitargs[0] == "renderer")
            {
                ...
                else if (splitargs.Length > 3 && splitargs[1] == "create")
                {
                    ...
                    obj.name = FindUniqueName(name);

                    Debug.Log(string.Format("Object {0} ({1}) created!", obj.name, objectType));
                }
    ...
    protected static string FindUniqueName(string name)
    {
        ...
        while (true)
        {
            if (GameObject.Find(checkName) == null)
                return checkName;
            else
                checkName = string.Format("{0}{1}", name, curSuffix++);
        }
    }
