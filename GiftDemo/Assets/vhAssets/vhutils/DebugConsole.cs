...
public class DebugConsole : DebugLogger
{
    ...
    public float InputFieldHeight
    {
        ...
        set
        {
            ...
            VHUI.StretchToParent(m_ScrollRect.GetComponent<RectTransform>(), 0, value, 0, 0);
    ...
    public void Awake()
    {
        ...
        Canvas canvas = VHUI.CreateCanvas("DebugConsoleCanvas", this.gameObject, m_CanvasSortingOrder);

        // add ugui elements to the canvas
        // main panel
        m_MainPanel = VHUI.CreateImage("DebugConsoleUI", canvas.transform);
        m_MainPanel.color = new Color(0, 0, 0, 100f / 255f);
        VHUI.StretchToParent(m_MainPanel.rectTransform, 0, 0, 0, 0);
        m_MainPanel.rectTransform.pivot = new Vector2(0.5f, 0);

        PercentageOfScreenTall = m_PercentageOfScreenTall;

        // create scroll rect
        m_ScrollRect = VHUI.CreateScrollRect("Scroll View", m_MainPanel.transform, true);
        m_ScrollRect.scrollSensitivity = 10;
        VHUI.StretchToParent(m_ScrollRect.GetComponent<RectTransform>(), 0, Screen.height /** m_InputPercentageTall*/, 0, 0);
        ...
        // create input field
        m_InputField = VHUI.CreateInputField("ConsoleInput", m_MainPanel.transform, (string s) => InvokeCommand(s));
        ...
        VHUI.SetAnchors(fieldRect, 0, 0, 1, 0);
    ...
    List<string> BreakUpTextLine(string text)
    {
        ...
        text = VHUI.InsertBreaks(text, m_HolderText.font, m_HolderText.fontSize, (int)((float)Screen.width * 0.95f));
